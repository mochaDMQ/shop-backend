...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    allowed.includes(ext) ? cb(null, true) : cb(new Error('Invalid image format'))
  },
...
router.get('/', (req, res) => {
  const rows = req.query.catid
    ? db.prepare('SELECT * FROM products WHERE catid = ? ORDER BY pid').all(req.query.catid)
...
router.get('/:id', param('id').isInt(), validate, (req, res) => {
  const row = db.prepare('SELECT * FROM products WHERE pid = ?').get(req.params.id)
...
router.put(
  ...
  async (req, res) => {
    try {
      const existing = db.prepare('SELECT * FROM products WHERE pid = ?').get(req.params.id)
      if (!existing) return res.status(404).json({ error: 'Product not found' })

      const catid       = req.body.catid       ?? existing.catid
      const name        = req.body.name        ?? existing.name
      const price       = req.body.price       ? parseFloat(req.body.price) : existing.price
      ...
      if (req.file) {
        // 需求5：重新上传时重新生成大图 + 缩略图（覆盖旧文件）
        const paths = await processAndSaveImages(req.file.buffer, req.params.id)
      ...
      ).run(catid, name, price, description, image_path, image_thumb_path, req.params.id)

      res.json({
        pid: Number(req.params.id),
        catid: Number(catid), name, price, description,
...
router.delete('/:id', param('id').isInt(), validate, (req, res) => {
  // 同时清理磁盘上的图片文件
  const row = db.prepare('SELECT image_path, image_thumb_path FROM products WHERE pid = ?').get(req.params.id)
  if (row) {
  ...
  const result = db.prepare('DELETE FROM products WHERE pid = ?').run(req.params.id)
